...
> insert into test2(name,tel)
  ...
  /* 두 테이블의 컬럼 타입만 같으면 컬럼명은 일치하지 않아도 된다. */
...
## autocommit
mysql은 autocommit의 기본 값이 true다. 따라서 명령창에서 SQL을 실행하면 바로 실제 테이블에 적용된다.
수동으로 처리하고 싶다면 autocommit을 false로 설정하라! 기업에서는 보통 autocommit 기능을 꺼둔다.
...
> rollback;
Temp에 임시 보관된 작업 결과를 실제 Database에 적용한다.
* 'delete', 'insert', 'update'는 Temp에 보관되지만 'drop' 처리 결과는 보관되지 않고 바로 삭제된다.
