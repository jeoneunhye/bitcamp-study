...
### 조회할 때 조건 지정하기
- where절과 연산자를 이용하여 조회 조건을 지정할 수 있다.
...
> select no, name, class, working
from test1
where not working = 'Y'; /* where working = 'N'; */
...
/* 학생 번호가 3의 배수인 경우 전화 번호를 '2222'로 변경하라 */
> update test1 set
  tel = '2222'
where (no % 3) = 0;
...
/* class 이름에 java를 포함하는 모든 학생 조회하기 
   이 경우 조회 속도가 느리다.*/
> select *
from test1
where class like '%java%';
...
/* 두 날짜 사이의 간격을 알아내기 */
datediff(날짜1, 날짜2);
> select datediff('2020-2-19', curdate());
...
select date_format(now(), '%p %h %H %l'); /* PM 01 13 1 h:12시간제 H: 24시간제*/
...
/* 날짜 값을 저장할 때 기본 형식은 'yyyy-MM-dd'다. */
...
> select no, title, date_format(regdt, '%m.%d') from test1;