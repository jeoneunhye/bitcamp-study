...
// => 서버와 연결할 때,
// 사용자 인증(authentication; 아이디, 암호 유효여부 검사)이나
// 사용 권한 확인(authorization; 사용자에게 허락된 작업 범위를 확인) 같은 작업을
// 반드시 수행하는 경우 연결하는 데 더욱 많은 시간이 걸린다.
// - 장점:
// 서버에 계속 연결된 상태가 아니기 때문에 서버 입장에서는 메모리를 많이 사용하지 않는다.
// 왜?
// 클라이언트와 연결을 계속 유지하지 않기 때문에 작업을 처리하는 동안만 연결 정보를 유지한다.
// 그래서 같은 메모리라도 stateful 방식보다는
// 더 많은 클라이언트의 요청을 처리할 수 있다.
// 비영속적인 단일 작업을 처리할 때 적합한 통신 방식이다.
