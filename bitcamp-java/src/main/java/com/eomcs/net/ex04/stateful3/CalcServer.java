...
public class CalcServer {
  ...
  public static void main(String[] args) throws Exception {
    ...
    while (true) {
      ...
      // System.out.println(socket.getRemoteSocketAddress() instanceof SocketAddress); // true
      // System.out.println(socket.getRemoteSocketAddress() instanceof InetSocketAddress); // true

      InetSocketAddress remoteAddr = (InetSocketAddress) socket.getRemoteSocketAddress();
      System.out.printf("클라이언트(%s:%d)가 연결되었음!\n",
          remoteAddr.getAddress(), remoteAddr.getPort());
      // 추상 클래스 SocketAddress의 서브 클래스인 InetSocketAddress의 메서드를 호출
      // 연결된 상대방의 IP Address와 port 번호를 출력
      ...
      requestHandler.start(); // run()이 아니다.
