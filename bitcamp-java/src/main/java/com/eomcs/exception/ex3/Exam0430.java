...
public class Exam0430 {
  ...
  public static void main(String[] args) { // 방법 1과 달리 main()에서 예외를 받지 않음
    // 예외 처리 방법 2:
    // try ~ catch를 사용하여 코드 실행중에 발생된 예외를 중간에 가로챈다.
    try {
      // try 블록에서 예외가 발생할 수 있는 코드를 둔다.
      m(1);
      System.out.println("실행 성공!");
      // try 블록에 있는 코드를 실행하는 중에 예외가 발생하면
      // 그 예외 객체를 파라미터로 받을 수 있는
      // catch문을 찾아 실행한다.
    } catch (IOException e) {
      // catch 블록에서 그 예외를 받아서 처리한다.
      // 메서드가 던지는 예외 갯수만큼 catch 블록을 선언하면 된다.
