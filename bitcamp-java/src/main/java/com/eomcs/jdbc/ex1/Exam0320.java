...
public class Exam0320 {
  public static void main(String[] args) throws Exception {
    try (
        ...
        // executeQuery()
        // => DBMS 서버에 select문을 보낸다.
        // => 리턴값: java.sql.ResultSet 구현 객체
        // ResultSet?
        // => 결과가 아니다! 서버에서 결과를 가져오는 일을 할 객체다.
        // => 즉 서버의 select 실행 결과를 가져올 때 사용하는 도구다.
      ...
      if (isReceived) {
        ...
        // => float: getFloat()
        // getXxx(컬럼번호)
        // => select문에 나열한 컬럼의 순서를 지정한다.
        // 단 번호는 0부터가 아니라 1부터 지정한다.
        // => select문에 wildcard(*)를 사용했다면,
        // 테이블을 정의할 때 선언한 순서대로다.

        // 컬럼의 타입과 상관없이 getString()으로 값을 꺼낼 수 있다.
        // 유지보수를 위해 되도록이면 타입에 맞춘다.
