...
// => 이 클래스를 사용하려면 먼저 이 JDBC Driver 파일을 다운로드받아
...
// > gradle eclipse
// - 다운로드받지 않은 라이브러리가 있다면 자동으로 서버에서 받을 것이다.
