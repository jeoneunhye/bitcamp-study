...
public class Account {
  ...
  // => 이 메서드처럼 여러 스레드가 접근했을 때 문제가 발생하는 critical section이 아닌데도
  // synchronized를 사용한다면 실행 속도가 떨어질 것이다.

  // 참고!
  // 여러 스레드가 동시에 실행해도 문제가 없는 코드 블록(synchronized를 붙이지 않아도 되는 코드)을
  ...
  synchronized public long withdraw(long money) {
    ...
    // 한 번에 한 스레드만 진입할 수 있도록 lock을 건다. (다른 스레드의 진입을 제한)

    // 코드 사이의 for문: CPU를 뺏어서 다른 스레드에게 기회를 제공하기 위함
    // 참고) volatile: 필드가 long이라면, 자바의 기본값인 4바이트를 두 번 읽어야 하기 때문에
    // 중간에 기회를 뺏겨 잘못된 수로 읽을 위험이 있다. 그것을 방지하기 위한 문법
    long b = this.balance;
    ...
    for (int i = 0; i < 1000; i++)
      Math.asin(45.765);
    this.balance = b; // <= critical section
    for (int i = 0; i < 1000; i++)
      Math.asin(45.765);
    return money;
  }
