...
public class Exam0110 {
  public static void main(String[] args) {
    Account account = new Account("111-11-1111-111", 100_0000); // 100만원을 입금
    ...
    강남.start(); // 스택 메모리에 로컬 변수 money와 sum을 저장하고 반복문 withdraw 계속 호출
    서초.start();
    부산.start();
    대전.start();
    광주.start();

    // 하나의 변수에 동시 접근했을 때 의도치 않은 결과가 나올 수 있다.
    // ATM이라는 같은 객체의 변수를 공유하기 때문에 여러 ATM에서 계좌를 출금했을 때
    // 작업 실행 속도가 각각 다르기 때문에 각각의 b값이 balance에 담기는 속도가 달라서
    // 잔액이 제대로 계산되지 않는 문제가 발생한다.
    // 스레드들이 처리한 값을, 공유하는 하나의 변수에 '저장(변경)'하려고 할 때 이런 문제가 발생한다.
    // account method area 스레드 stack
    // 강남, 서초 등 여러 개의 스레드가
    ...
    //
    // 해결책?
    // => 한 번에 한 스레드만이 크리티컬 섹션을 실행하도록(변수의 값을 변경하도록)
    //    접근을 제한하면 된다.
    // 주의!
...
// - 여러 스레드가 같은 메모리에 접근하더라도 '읽기'만 한다면 아무런 문제가 되지 않는다.
...
// - semaphore(1)과 같다.
// - 자바에서 synchronized는 해당 블록(critical section)을 뮤텍스로 선언하는 것이다.