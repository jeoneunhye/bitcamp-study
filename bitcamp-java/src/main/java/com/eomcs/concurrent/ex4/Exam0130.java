...
public class Exam0130 {
  public static void main(String[] args) throws Exception {
    ...
    // => 즉 3초동안 CPU가 놀고 있더라도 CPU를 사용하지 않는다.
    // => 3초가 지나면(timeout) 다시 "main" 스레드는 CPU를 받아 실행할 수 있다.
    // Thread.currentThread().sleep(3000); // currentThread: main 스레드를 가리킨다.
    Thread.sleep(3000); // 메인 메서드에 있는 메인 스레드를 sleep. 스태틱 메서드기 때문에 생략 가능
