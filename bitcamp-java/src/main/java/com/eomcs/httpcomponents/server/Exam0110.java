// Apache HttpClients 사용법 : 파일을 리턴해주는 웹서버 만들기
...
public class Exam0110 {
  public static void main(String[] args) throws Exception {
    ...
    final HttpServer server = ServerBootstrap.bootstrap()
        ...
        // https 요청이 들어왔을 때 처리하도록 -> 우리는 http이기 때문에 SSL을 사용하지 않는다.
  ...
  // 웹서버에서 예외가 발생할 때 그 예외를 처리하는 일을 한다.
  ...
  static class HttpFileHandler implements HttpRequestHandler {
    ...
    @Override
    public void handle(
        final ClassicHttpRequest request, // 클라이언트 요청 도구
      ...
      // 클라이언트가 요청한 자원의 경로
      // 예) HTTP 요청 URL: http://localhost:9999/board/list
      //     자원의 경로: /board/list
      final String path = request.getPath();

      final HttpEntity incomingEntity = request.getEntity();
      if (incomingEntity != null) { // 클라이언트가 보낸 데이터가 있다면 읽는다.
      ...
      // => ./webroot + /index.html을 요구했다면
      // => ./webroot/index.html을 준비한다.
