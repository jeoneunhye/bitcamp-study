// Apache HttpComponents 사용법 : HttpClient5 - GET 요청하기 I
...
// HttpClients, HttpGet 등 클래스가 있다.
// => www.apache.org 사이트에서 제공하는 HttpComponents 라이브러리에 들어있는 클래스다.
// => 사용하려면 먼저 외부 라이브러리를 프로젝트로 가져와야 한다.
//  1) mvnrepository.com 또는 search.maven.org에서 'httpclient5' 키워드로 검색한다.
//  2) 최신 라이브러리 정보를 build.gradle의 dependencies {} 블록에 추가한다.
//     implementation 'org.apache.httpcomponents.client5:httpclient5:5.0'
//  3) 콘솔에서 $ gradle eclipse를 실행하여 .classpath 파일을 갱신한다.
//     이때 외부 라이브러리를 자동으로 다운로드할 것이다.
//  4) 이클립스 에디터에서 해당 프로젝트를 리프레시한다.
//  5) 이클립스의 프로젝트에서 해당 라이브러리가 추가되었는지 확인하라!
public class Exam0110 {
  public static void main(String[] args) throws Exception {
    // => createDefault라는 factory method를 이용하여 HTTP 요청을 수행할 객체를 준비한다.
    ...
    if (entity != null) {
      ...
      System.out.printf("응답 데이터 크기 => %d\n",
          entity.getContentLength()); // 바이트 형식. 못 꺼내는 경우도 있다.
      System.out.printf("응답 데이터의 타입 => %s\n",
          entity.getContentType()); // Content-Type: text/html;charset=UTF-8
