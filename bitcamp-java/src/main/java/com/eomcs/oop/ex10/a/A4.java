...
public abstract class A4 {
  ...
  // 의미?
  // => 서브 클래스마다 구현이 다를 경우 굳이 수퍼 클래스에서 정의할 필요가 없기 때문에
  //    완전히 정의하지 않은 추상 메서드로 남겨둔다.
  // => 서브 클래스에게 메서드 구현을 강제하고 싶을 때도 추상 메서드를 활용한다.
  //    추상 메서드를 상속받은 서브 클래스는 반드시 정의해야 하기 때문이다.
  //    서브 클래스가 상속받은 추상 메서드를 구현하지 않는다면
  //    추상 메서드를 그냥 보유하기 때문에 일반 클래스가 될 수 없다.
  //    즉 추상 클래스가 되어야 한다.
  // "이 메서드는 수퍼 클래스에서 정의해 봐야 소용없어. 어차피 서브 클래스에서 재정의할 메서드야!"
