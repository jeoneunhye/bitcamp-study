...
// 방법2)
//    => 방법1처럼 기존 규칙을 상속받아서 새 규칙을 만들면
//       계속 규칙이 늘어나 관리하기가 어려운 문제가 있다.
//       그리고 규칙에 따라 구분하도록 코딩해야 하기 때문에 프로그래밍하기가 불편하다.
//       기존 규칙에 새 규칙을 추가한다면 이런 문제가 제거된다.
//    => touch 기능을 기존 규칙(Computer)에 추가한다. void touch();
//    => 문제는 인터페이스에 touch()라는 새 규칙을 추가하게 되면
//       기존에 구현한 클래스(First~ThirdComputer)는 모두 오류가 발생할 것이다.
//       왜?
//       규칙이 달라졌기 때문에 클래스 구현 오류가 발생하는 것이다.
//       클래스는 항상 인터페이스의 모든 메서드를 구현해야 하기 때문이다.
//       => Computer.java를 확인해 보자
