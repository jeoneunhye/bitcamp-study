// Object 클래스의 메서드를 오버라이딩하기 - hashCode()
...
public class Exam0230 {
  static class Score {
    ...
    public int hashCode() {
      ...
      //     같은 해시 값을 리턴하도록 이미 오버라이딩 되어있기 때문이다.
      String value = String.format("%s,%d,%d,%d,%d,%.1f", 
          this.name, this.kor, this.eng, this.math,
          this.sum, this.aver); // 이름, 국, 영, 수를 받아 문자열로 만들고
      return value.hashCode();  // 해시코드를 리턴
    }
    // 데이터가 같으면 문자열이 같을 것이고,
    // 문자열이 같으면 해시코드의 리턴값도 같을 것이다.
  }
  ...
  public static void main(String[] args) {
    ...
    // hashCode()를 왜 오버라이딩한다고?
    // => 두 개의 인스턴스가 같은 데이터인지 비교하기 위해서
    
    // 해시코드를 이용하는 예) 카드키, 공인인증서, ...
  }
}