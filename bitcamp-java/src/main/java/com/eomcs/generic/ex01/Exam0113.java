...
public class Exam0113 {
  ...
  static class Box<T> {
    ...
    // 타입 파라미터도 변수기 때문에 이름을 다음과 같이 지어도 된다.
  ...
  public static void main(String[] args) {
    ...
    // 인스턴스를 생성할 때에도 어떤 타입의 값을 다룰 인스턴스인지 지정해야 한다.
    box1 = new Box<Member>();
    ...
    // 따라서 다음과 같이 값을 꺼낼 때에도 따로 형변환할 필요가 없다.
    Member m = box1.get();
    ...
    //box1.set(new String("Hello")); // 컴파일 오류 Member!=String
