...
public class Exam0110 {
  public static void main(String[] args) {
    ...
    set.add(v4);
    set.add(v5);
    // v5와 v3는 다른 인스턴스다. 하지만 두 객체에 들어있는 문자열은 같다.
    // String 클래스는 인스턴스가 다르더라도 값이 같으면
    // hashCode()의 리턴 값이 같도록 오버라이딩하였다.
    // 또한 equals()의 리턴 값도 true가 되도록 오버라이딩하였다.

    // HashSet은 값의 중복을 검사할 때 인스턴스의 주소를 비교하지 않고
    // hashCode()의 리턴 값과 equals()의 리턴 값으로 판단한다.
    // 따라서 v5는 비록 v3와 다른 인스턴스지만,
    ...
    System.out.println(v3.hashCode() + "," + v5.hashCode()); // hashCode 일치
    System.out.println("---------------------");
    print(set);
  }
