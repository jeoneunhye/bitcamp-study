...
public class Exam0155 {
  public static void main(String[] args) {
    ...
    System.out.println(map.get(k3)); // Student [name=유관순, age=17, working=true]
    
    // k3랑 같은 문자열을 갖는 String key를 생성한다.
    String k6 = new String("haha");

    // k3랑 k6는 서로 다른 인스턴스이다.
    System.out.println(k3 == k6); // false
    
    // 그러나 문자열은 같다.
    // String은 같은 문자열일 경우 같은 해시코드를 리턴하도록 메서드를 오버라이딩 하였다.
    System.out.println(k3.hashCode()); // 3194802 hash code는 같다.
    System.out.println(k6.hashCode()); // 3194802 hash code는 같다.
    
    // 또한 문자열이 같을 경우 equals()의 리턴 값이 true가 되도록 오버라이딩 하였다. 
    System.out.println(k3.equals(k6)); // true equals()의 비교 결과도 같다.
    
    // 즉 k3와 k6는 서로 다른 인스턴스지만,
    // hashCode()의 리턴 값이 같고, equals()의 리턴 값이 true기 때문에
    // 두 객체는 같은 key로 간주한다.
    ...
    // 당연히 hashCode()의 리턴 값이 다르고, equals()의 리턴 값은 false다.
