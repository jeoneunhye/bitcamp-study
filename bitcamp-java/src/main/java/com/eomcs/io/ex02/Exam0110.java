...
public class Exam0110 {
  public static void main(String[] args) throws Exception {
    ...
    out.write(0x7a6b5c4d); // 출력하는 값은 0x4d이다. => visual studio code의 show Hexdump로 확인

    // 3) 출력 도구를 닫는다.
    // - OS에서 관리하는 자원 중에서 한정된 갯수를 갖는 자원에 대해
    //   여러 프로그램이 공유하는 경우에는 사용 후 OS에 반납해야 한다.
    //   그래야 다른 프로그램이 해당 자원을 사용할 수 있다.
    //   ex) 파일, 메모리, 네트워크 연결 등
    // - 이런 자원을 사용하는 클래스는 항상 자원을 해제하는 close()라는 메서드가 있다.
    // - 이번 예제에서 다루는 FileOutputStream 클래스에도 close()가 있다.
    // - 따라서 FileOutputStream 객체를 사용한 후에는 close()를 호출하여
    //   사용한 자원을 해제시켜야 한다.
    // - close()를 호출하면
    //   FileOutputStream이 작업하는 동안 사용했던 버퍼(임시 메모리)를 비운다.
    ...
    // 자원 부족 문제가 발생한다.
    // 결론!
    // 간단한 코드를 작성하더라도 close()를 호출하는 것에 게을리 하지 말라!
    System.out.println("데이터 출력 완료!");
  }
}

// 파일 입출력 API 주요 클래스 (java.io 패키지 중)
// 1) 데이터 읽기
//    InputStream (추상 클래스)
...
// ex) .pdf, .ppt, .xls, .gif, .mp3, .jpg, .hwp, .mov, .avi, .exe, .lib 등
...
// ex) .txt, .csv, .html, .js, .css, .xml, .bat, .c, .py, .php, .docs, .pptx, .xlsx 등
