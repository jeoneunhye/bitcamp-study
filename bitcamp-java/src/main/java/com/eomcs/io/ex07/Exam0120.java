// 메모리에서 데이터를 읽기 - 읽은 데이터를 Member 객체에 저장하기
...
public class Exam0120 {
  public static void main(String[] args) throws Exception {
    ...
    // 바이트 배열의 값을 읽어서 String이나 int, boolean값으로 만드려면
    // 다음과 같이 비트 이동 연산 등을 수행해야 한다.

    // 1) 문자열 읽기
    // 문자열 바이트의 크기(length)를 먼저 읽는다.
    int size = in.read(); // 0x0b = 11(바이트의 문자열)
    ...
    // 해결책?
    // ByteArrayInputStream을 상속받아 이런 기능을 수행하는 메서드를 추가하라!
    // ex05의 DataInputStream 클래스처럼 만들라!
  }
}