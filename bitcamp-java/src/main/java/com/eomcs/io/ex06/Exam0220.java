...
public class Exam0220 {
  public static void main(String[] args) throws Exception {
    ...
    System.out.println(endTime - startTime); // 50
...
// 2) 1000바이트 1번 읽는 경우
//    0.0105 * 0.00000005초 * 1000byte = 0.01055초
// 1바이트를 여러 번 읽을 경우 매번 바이트의 위치를 찾아야 하기 때문에
// 평균 탐색시간이 누적되어 한 번에 1000바이트를 읽는 것보다 시간이 오래 걸린다.
//
// 그러면 RAM에 배열의 크기를 크게 잡으면 좋지 않겠는가?
// => PC처럼 소수의 프로그램이 동시에 실행될 때는 상관없지만,
//    서버처럼 데이터를 읽는 코드가 동시에 수천 개에서 수십만 개일 때는
