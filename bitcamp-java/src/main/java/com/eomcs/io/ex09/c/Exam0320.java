...
public class Exam0320 {
  public static void main(String[] args) throws Exception {
    ...
    // Exam0320에서 Member 데이터를 읽는다.
    // => 결과: 정상적으로 읽을 수 있다.

    // test2
    // Exam0310에서 Member 객체를 출력한다.
    // Member 클래스에 인스턴스 필드 'tel'을 추가한다.
    // toString()에도 'tel'을 추가한다.
    // Exam0320에서 Member 데이터를 읽는다.
    // => 결과: InvalidClassException 실행 오류가 발생한다.
    // => 이유?
    // DeSerialize할 때,
    ...
    // => 검사 방법?
    // Serialize할 때 사용한 Member 클래스와
    // DeSerialize할 때 사용한 Member 클래스가 같은지 검사하는 방법
    // 클래스에 내장된 스태틱 변수인 serialVersionUID의 값을 비교한다.
    // java.io.Serializable을 구현한 객체 클래스는 항상 이 변수를 내장한다.
    ...
    // Member 클래스를 변경하니 컴파일러가 이 클래스의 serialVersionUID 변수의 값을 변경하였다.
    ...
    // 즉, JVM은 저장할 때 메모리 형식과 읽을 때 형식이 다르다고 판단하여
    // 실행 오류를 띄웠다.
  }
}