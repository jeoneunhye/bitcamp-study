...
public class Exam01_3 {
  public static void main(String[] args) {
    ...
    //z = x + y; // 컴파일 오류! : 인트를 바이트로 바꿀 수 없다. 변수끼리는 X
    // "자바는 정수 연산은 최소 단위가 4바이트이다."
    // " 그래서 byte나 short의 연산 단위가 기본으로 4바이트이다."

    // - 자바가 정수 변수에 대해 산술 연산을 수행할 때,
    //   그 변수의 값이 4바이트 보다 작다면, (byte나 short라면)
    //   4바이트로 만들어 연산을 수행한다.
    // - 즉 임시 4바이트 정수 메모리를 만든 다음에 (기존의 x, y는 4바이트가 아닌 여전히 1바이트다)
    ...
    int s3 = s1 + s2;   // 메모리 낭비를 감수하고 int 변수 사용
