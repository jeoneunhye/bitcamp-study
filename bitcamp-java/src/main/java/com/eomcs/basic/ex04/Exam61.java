...
public static void main(String[] args) {
  ...
  // score[-1] = 100;  // runtime error! : outofbounds
  // score[5] = 100; // outofbounds 범위를 벗어남

  // 배열 레퍼런스와 인스턴스를 따로 선언
  int[] arr1 = null;  // 아래 에러를 해결하기 위해 null을 써보지만 그래도 안됨. 주소가 0 => 의미? 메모리를 가리키지 않음
  ...
  arr2[0] = 300;
  System.out.println(arr1[0]);  // 300★ 결국은 같은 배열을 가리키기 때문에 같은 값이 나옴

  // 배열 인스턴스의 각 항목은 생성되는 순간 기본값으로 자동 초기화된다. -> 일반 변수와 다르다
  System.out.println(arr1[2]);  // 0 초기화 안시켰지만 에러가 안뜸
  // byte, short, int, long 배열 : 0
  // float, double 배열 : 0.0
  // boolean 배열 : false;
  // char 배열 : \u0000;

  int[] arr3 = new int[3];  //레퍼런스
  arr3[0] = 30;
  arr2 = arr3;
  ...
  //    가비지 또한 반납될 것이다.
  // => 전산학에서는 "가비지"를 "dangling object" 라고 부른다(덩그러니)
  ...
  // 초기화시키는 값 개수만큼 배열 항목이 생성된다.
  int[] arr5 = new int[] {100, 90, 80};
  // 인스턴스 생성과 동시에 값을 지정할 때는
  // 배열 개수를 지정하면 안 된다.
  // int[] arr6 = new int[3] {100, 90, 80}; // 컴파일 오류 illegal

  // 배열 선언과 동시에 인스턴스를 초기화시킬 때는
  // new int[]를 생략할 수 있다.
  int[] arr7 = {100, 90, 80}; // 실무에서 사용
