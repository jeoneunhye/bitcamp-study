...
- 인터페이스를 이용하면 메서드 호출 규칙을 단일화할 수 있기 때문에
  코딩의 일관성을 높여준다.
- 단 기능 추가할 때마다 해당 기능을 처리하는 새 클래스가 추가되기 때문에
  클래스 갯수는 늘어난다.
- 그러나 유지보수 측면에서는 클래스 갯수가 늘어나는 것이 중요하지 않다.
- 소스 코드를 일관성있게 유지보수할 수 있는 것이 더 중요하다.
...
### 훈련1. 메서드를 호출하는 쪽과 실행하는 쪽 사이의 규칙을 정의하라.
