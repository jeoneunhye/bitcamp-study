...
public class Queue<E> extends LinkedList<E> implements Cloneable {
  ...
  public E poll() {
    return (E) this.remove(0); // 값을 조회하는 것이 아닌 아예 꺼내는 것(!=get)
  }
  ...
  // 이 때 원본 객체의 노드의 링크 정보를 변경하게 된다.
  // 따라서 다음에 다시 복제를 수행하면 원본 노드의 링크 정보가 제거되었기 때문에
  // 제대로 작업을 수행할 수 없다.
  // 해결책?
  ...
  public Queue<E> clone() {
    // 'deep copy' 수행
    // 스택의 경우 배열을 복사하면 되기 때문에 기존의 shallow copy를 수행한 후
    // 배열을 따로 복사하였다.
    // LinkedList의 경우 노드와 노드 사이를 연결해야 하기 때문에
    // 단순히 shallow copy를 수행해서는 안 된다.
