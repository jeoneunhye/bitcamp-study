...
//    서브 클래스를 정의하는 개발자가 해당 메서드를 오버라이딩한다는 보장을 못한다.
// => 상속받는 메서드 중에서 반드시 서브 클래스에서 오버라이딩해야 할 메서드라면
//    문법으로 표시를 하는 것이 좋다.
//    이런 용도로 만든 문법이 "추상 메서드"다.
// => 추상 메서드의 오버라이딩은 선택사항이 아니라 필수 사항이다.
// => 따라서 서브 클래스를 만드는 개발자는 반드시 이 추상 메서드를 구현해야 한다.

// 추상 메서드
// => 서브 클래스가 반드시 재정의해야 할 메서드라면 수퍼 클래스에서 정의하지 않는다.
// => 또한 서브 클래스가 반드시 해당 메서드를 오버라이딩하도록 강제하고 싶을 때 사용하는 문법이다.
