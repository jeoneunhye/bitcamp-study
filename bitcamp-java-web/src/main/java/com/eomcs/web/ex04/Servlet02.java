...
public class Servlet02 extends GenericServlet {
  ...
  public void service(ServletRequest req, ServletResponse res)
    ...
    // 테스트
    // http://localhost:9999/bitcamp-java-web/ex04/test02.html 실행
    ...
    // => 다음 코드의 주석을 풀고 테스트해보라!
    //    정상적으로 잘 출력될 것이다.
    // => 원리
    //    getParameter()를 최초로 호출하기 전에 먼저
    //    클라이언트 보낸 데이터의 인코딩 형식이 어떤 문자표로 되어있는지 알려줘야 한다.
    // => 주의!
    //    반드시 getParameter()를 최초로 호출하기 전이어야 한다.
    //    한 번 getParameter()를 호출한 후에는 소용없다.

    req.setCharacterEncoding("UTF-8"); // 주석처리하면 이름=ABCê°ê° 나이=20 => 입력의 문제

    String name = req.getParameter("name");
    String age = req.getParameter("age");
    // GET 방식과 달리 UTF-8로 인코딩된 데이터라고 미리 알려줘야 한다.
    // 낱개로 하지 않고 묶어서 AC00으로 처리하도록 setCharacterEncoding("UTF-8")을 호출한다.
    //      ISO-8859-1   =>  URL Encoding   =>   UCS2
    //  가    EAB080        00EA 00B0 0080       AC00
...
// => 예)
/*
      POST /bitcamp-java-web/ex04/s2 HTTP/1.1
      Host: localhost:9999
      Connection: keep-alive
      Content-Length: 33 // 요청한 내용의 길이(byte)
      Pragma: no-cache
      ...
      Referer: http://localhost:9999/bitcamp-java-web/ex04/test02.html
...
// => 용도
//    게시글 조회나 검색어 입력같은 간단한 데이터 전송에는 GET 요청으로 보내고
