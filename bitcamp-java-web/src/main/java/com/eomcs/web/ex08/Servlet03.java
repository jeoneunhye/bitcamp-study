...
public class Servlet03 extends HttpServlet {
  ...
  protected void service(HttpServletRequest request, HttpServletResponse response)
    ...
    // 테스트 방법:
    // http://localhost:9999/bitcamp-java-web/ex08/s3
    ...
    // 리다이렉트가 안먹힌다.
    // 왜? 리다이렉트 명령을 응답 헤더로 보내기 때문이다.
    // 이미 클라이언트로 출력을 완료했는데 어떻게 응답 헤더를 보내는가?
    // 다음 반복문 주석을 풀라!
    ...
    // forward/include와 달리 '/'는 컨텍스트 루트(웹 애플리케이션 루트)가 아닌
    // 웹 서버 루트를 의미한다.
...
// HTTP/1.1 302                <----- 요청한 자원이 다른 URL에 있음을 알려 준다.
// Location: s100              <----- 다른 URL의 주소를 알려 준다.
// Content-Type: text/html;charset=UTF-8
// Content-Length: 0
// Date: Tue, 07 Apr 2020 07:04:52 GMT
