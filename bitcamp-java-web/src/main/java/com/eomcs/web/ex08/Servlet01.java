// 리프래시 I - 클라이언트에게 다른 URL을 요청하라는 명령
...
public class Servlet01 extends HttpServlet {
  ...
  protected void service(HttpServletRequest request, HttpServletResponse response)
    ...
    // 테스트 방법:
    // http://localhost:9999/bticamp-java-web/ex08/s1
    ...
    //    메서드 호출이 완료될 때 비로소 클라이언트로 응답 헤더와
    //    버퍼에 저장된 message-body가 출력된다.

    // 만약 out.println()/out.printf()/out.print() 등에서 출력한 내용이
    // 버퍼를 꽉 채웠다면 어떻게 하나요?
    // => 그러면 자동으로 클라이언트에게 응답한다.
    //    따라서 일단 클라이언트에게 응답하면 헤더를 추가하거나 변경하는 코드는 적용되지 않는다.
    //    응답을 완료한 후에 헤더 값을 변경하거나 바꿀 수 없기 때문이다.

    // 다음은 일부러 버퍼를 채우는 코드이다.
    // 버퍼가 꽉차면 자동으로 출력하는 것을 확인해 보자!
    for (int i = 0; i < 200; i++) {
      // 약 50바이트씩 100번 출력하면 아직 버퍼에 차지 않았기 때문에
      // 클라이언트로 출력되지 않는다.
      // 따라서 반복문 아래에 있는 응답 헤더 설정이 유효하다.
      // 그러나 200번 출력하면 헤더 값과 이전에 출력한 값,
      // 그리고 반복문에서 출력한 데이터가 8KB 버퍼가 꽉 채우기 때문에
      // 즉시 클라이언트로 응답한다. 응답 헤더에 리프래시가 없다.
      // 즉 반복문 다음에 헤더를 설정하기 전에 이미 버퍼 내용이 출력된다.
      // 응답을 완료한 다음에 응답 헤더의 값을 변경하거나 추가해 봐야 소용없다.
      out.println(i + "  ===> 1234567890123456789012345678901234567890"); // 숫자 40byte
    }

    response.setHeader("Refresh", "3;url=s100"); // 3초 후 url=s100(Servlet100)으로 이동한다.
...
/*
    HTTP/1.1 200
    Refresh: 3;url=s100 <- 웹 브라우저는 이 헤더의 정보에 따라 다시 요청한다. s100 = 상대 경로 => /ex08/s100
