...
@WebServlet(
    ...
    initParams={
        ...
        @WebInitParam(name="jdbc.url", value="jdbc:mariadb://localhost:3306/studydb"),
...
public class Servlet03 extends HttpServlet {
  ...
  //    // 이 메서드를 직접 오버라이딩하지 말고
  //    // 이 메서드가 호출하는 다른 init()를 오버라이딩하라!
  //    super.init(config);
  //  }
  ...
  public void init() throws ServletException {
    ...
    // 이 소스를 변경하고 다시 컴파일해야 하는 번거로움이 있다.
    ...
    // 위 코드처럼 언제든 변경될 수 있는 값을
    // 소스 코드에 직접 작성하는 방식은 좋지 않다.
    // 해결책?
  ...
  // 다른 서블릿을 위한 객체를 준비할 뿐 Mapping하지 않기 때문에 클라이언트가 이 페이지를 이용할 수 없다.
  // 이런 경우 service를 오버라이딩하지 않는다.
}