...
public class Servlet01 extends HttpServlet {
  ...
  protected void service(HttpServletRequest request, HttpServletResponse response)
    ...
    // 테스트 방법:
    // http://localhost:9999/bitcamp-java-web/ex10/s1

    // 쿠키
    // => 웹 서버가 웹 브라우저에게 맡기는 데이터다.
    // => 응답할 때 응답 헤더에 포함시켜 보낸다.
    // => 웹 브라우저는 응답 헤더로 받은 쿠키 데이터를 보관하고 있다가
    ...
    /*
          ...
          Set-Cookie: name2=홍길동  <---- URL 인코딩하지 않은 상태
    ...
    // 쿠키 생성
    // => 이름과 값으로 생성한다.
    // => 쿠키의 유효기간을 설정하지 않으면 웹 브라우저가 종료될 때까지 유지된다.
    //    웹 브라우저를 종료하면 유효기간이 지정되지 않은 쿠키는 모두 삭제된다.
    // => 쿠키의 사용 범위를 지정하지 않으면 현재 경로에 한정한다.
    //    쿠키를 보낼 때의 URL이 /ex10/s1이라면,
    //    웹 브라우저는 /ex10/* 경로를 요청할 때만 웹 서버에게 쿠키를 보낸다.
