...
- [ ] 컴퓨터에서 프로그램이 실행되는 원리를 이해한다. -> *.exe 속의 명령어를 cpu가 입출력하여 실행
- [ ] CPU(ARM CPU 기준) 인스트럭션이 무엇인지 이해한다. `http://vision.gel.ulaval.ca/~jflalonde/cours/1001/h17/docs/arm-instructionset.pdf`
- [ ] CPU 인스트럭션과 실행 파일, OS의 관계를 이해한다. -> CPU 인스트럭션에 모여 있는 실행 파일
- [ ] 같은 CPU에 맞춰 작성된 실행 파일이더라도 OS가 다르면 실행되지 않는 이유를 설명할 수 있다.

#### 중간 언어와 가상 머신

- [ ] 중간 언어와 가상 머신이 등장한 이유와 이점을 설명할 수 있다. -> 여러 os에서 사용하기 위해 진짜 기계어로 바꾸지 말고 플레이어를 이용해 실행하게 하자
- [ ] VM과 인터프리터, 런타임, 플레이어, 엔진의 역할을 이해한다.
- [ ] JIT(Just-in-time)과 AOT(ahead-of-time)이 무엇이고 등장한 이유를 이해한다.

#### 자바 바이트코드와 자바 가상 머신

- [ ] 자바 바이트코드 실행 환경을 준비할 수 있다. OpenJDK 설치:graalVM, JAVA_HOME 과 PATH 환경 변수 설정, Sublime 편집기 설치.
...
- [ ] 컴파일 방식 프로그래밍이 등장한 이유를 이해한다. -> 기계어 직접 작성 cafe babe에서 시작,,
...
- [X] 링커(C 프로그래밍 언어 기준)를 이용하여 목적 코드가 들어 있는 파일을 하나로 묶어 실행 파일을 만드는 과정을 이해한다. `$ ld -o 실행파일 목적파일 목적파일 … -lSystem -macosx_version_min 10.13` 하지마
...
- [X] 패키지를 적용하지 않은 채로 그 소스 파일을 그냥 패키지 폴더에 둔다면 컴파일 할 때 어떤 문제가 발생하는지 안다. `src/main/java/com/ohoracs/basic/Hello4.java`
      -> 클래스파일이 우리가 원하는 bin폴더로 저장되지 않고 java 폴더 내에 새로운 bin 폴더가 생성되어 그 안에 저장된다
~19/12/4
#### Gradle 빌드 도구
...
- [ ] 컴파일 할 때 소스 파일의 인코딩을 지정할 수 있다. `ex01/Exam3.java` -> 기본 msm949이므로 utf8로 변경 필요
- [X] JVM이 클래스를 실행하는 과정을 이해하고 설명할 수 있다. -> bytecode를 interpreting하여 실행 파일을 생성한다.
...
- [X] Javadoc 주석(`/** … */`)의 용도를 이해하고 이 주석을 이용하여 HTML 문서를 생성(`javadoc 사용`)할 수 있다. `ex02/Exam2.java`
- [ ] 애노테이션(`@Override`)의 용도와 문법을 이해한다. `ex02/Exam3.java`

#### 데이터 종류에 따른 리터럴 표기법

- [ ] `정수, 부동소수점, 논리, 문자, 문자열` 리터럴 문법의 용도를 이해한다. `ex03/Exam1.java`
- [ ] 정수 리터럴을 10진수, 8진수, 2진수, 16진수로 표현할 수 있다. `ex03/Exam21.java` ~19/12/05
...
- [X] UCS(ISO 10646)를 UTF-8로 바꾸는 규칙을 이해한다.
- [X] UCS 대신 UTF-8이 사용되는 이유를 안다.
- [ ] 문자를 다룰 때 작은 따옴표('')의 의미를 이해한다. `ex03/Exam43.java`
- [ ] 논리 리터럴을 표현할 수 있다. `ex03/Exam5.java`
- [ ] 문자열 리터럴을 표현할 수 있다. `ex03/Exam6.java`
- [X] 문자열과 다른 유형의 데이터를 결합할 때 어떤 결과가 발생하는지 안다. `ex03/Exam6.java`
- [X] 이스케이스 문자의 역할과 사용법을 안다. `ex03/Exam7.java`
...
- [ ] 배열 레퍼런스와 배열 인스턴스의 관계를 이해한다. `ex04/Exam62.java`
- [ ] 배열 레퍼런스와 배열 인스턴스를 분리해서 선언할 수 있다. `ex04/Exam621.java`
- [ ] 배열 레퍼런스를 초기화시키지 않고 사용했을 때 문제점을 이해한다. `ex04/Exam621.java`
- [ ] 다른 배열 레퍼런스를 이용하여 배열 인스턴스를 공유할 수 있다. `ex04/Exam622.java`
- [ ] 배열 메모리가 생성될 때 기본 값으로 자동 초기화 되는 것을 안다. `ex04/Exam63.java`
- [ ] 특정 값으로 초기화된 배열 인스턴스를 만들 수 있다. `ex04/Exam631.java`
- [ ] 배열 인스턴스가 가비지가 되는 상황을 이해한다. `ex04/Exam64.java`
- [ ] 상수를 선언하고 사용할 수 있다. `ex04/Exam71.java`
- [ ] 변수의 종류를 구분할 수 있다. `ex04/Exam81.java`
- [ ] 변수의 종류에 따라 사용할 수 있는 범위가 다름을 안다. `ex04/Exam82.java`
- [ ] 같은 클래스 블록이나 메서드 블록에서는 중복된 이름으로 변수를 선언할 수 없음을 안다. `ex04/Exam83.java`
- [ ] 메서드에서 블록에 선언된 변수는 블록 밖에서 사용할 수 없음을 안다. `ex04/Exam84.java`
- [ ] 메서드에서 블록에 변수를 선언할 때 블록 밖에 선언된 변수와 같은 이름으로 변수를 선언할 수 없음을 안다. `ex04/Exam84.java`